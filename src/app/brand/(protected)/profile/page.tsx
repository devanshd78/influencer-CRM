...
export default function BrandProfilePage() {
    ...
    const [formState, setFormState] = useState<Partial<BrandData>>({});
    ...
    useEffect(() => {
        ...
        get<BrandData>(`/brand?id=${brandId}`)
            .then((data) => {
                setBrand(data);
                setFormState({
                    name: data.name,
    ...
    const saveProfile = async () => {
        if (!brand) return;
        try {
            const updated = await post<BrandData>(
                `/brand?id=${localStorage.getItem("brandId")}`,
                {
                    name: formState.name,
                    email: formState.email,
                    phone: formState.phone,
                    country: formState.county,
                }
            );
            setBrand(updated);
            setIsEditing(false);
    ...
    const formattedExpiry = brand && brand.subscription.expiresAt
        ? new Date(brand.subscription.expiresAt).toLocaleDateString(undefined, {
            month: "short",
            day: "numeric",
            year: "numeric",
        })
                        ...
                        {!isEditing && (
                            <div className="text-right">
                                ...
                                <p className="text-xl font-semibold text-gray-800">
                                    $ {brand ? brand.walletBalance.toFixed(2) : "0.00"}
                                </p>
                            </div>
                        )}
                    </div>
                        ...
                        {[
                            {
                                ...
                                label: "Phone",
                                field: "phone",
                                prefix: brand?.callingcode,
                            },
                            { icon: HiMail, label: "Email", field: "email" },
                            { icon: HiGlobe, label: "Country", field: "country" },
                        ].map((item) => (
                            <div
                                key={item.label}
                                    ...
                                    {isEditing ? (
                                        <input
                                            ...
                                            value={
                                                item.field === "phone"
                                                    ? formState.phone
                                                    : (formState[item.field as keyof BrandData] as string)
                                            }
                                            onChange={(e) =>
                                                setFormState({
                                                    ...formState,
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-medium text-gray-700">
                                            {brand ? brand.subscription.name : ""}
                                        </span>
                                    </div>
                                    <div className="space-y-2">
                                        {brand && brand.subscription.features.map((feat) => {
